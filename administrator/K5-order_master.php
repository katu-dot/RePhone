...
if (isset($_GET['q']) && $_GET['q'] !== '') {
    ...
    $execute_search = true; // 検索モードON
} else {
    // 検索ワードがない場合 (初期表示時) は、全件表示のため検索を有効にする
    $like_query = '%%'; // 全てにマッチするワイルドカードを設定
    $execute_search = true; // 全件表示モードON
}

if ($execute_search) {
    try {
        // --- ▼ SQLクエリ (全件表示/検索 共通) ▼ ---
        // 検索ワードがない場合は LIKE '%%' が適用され、全件取得となる
        $sql = "
            SELECT 
                OM.*, 
        ...
        // プレースホルダに、検索ワードの有無にかかわらず $like_query をバインド
