...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['cancel_order'])) {
        try {
            $stmt = $pdo->prepare("UPDATE order_management SET cancelled_at = NOW(), cancel_request_status = NULL WHERE order_management_id = ?");
    ...
    // 3. キャンセル申請却下
    if (isset($_POST['cancel_reject'])) {
        try {
            $stmt = $pdo->prepare("
                UPDATE order_management
                SET cancel_request_status = NULL,
                    cancel_reason = NULL,
            ...
            $subject = "【RePhone】キャンセル申請が却下されました";
            $body = $name . " 様\n\n"
                  . "ご注文ID：" . $order_id . "\n"
                  . "キャンセル申請は却下されました。\n\n"
                  . "ご不明点がございましたらお問い合わせください。";
            ...
            echo "<div class='notification is-danger'>却下処理エラー: " . htmlspecialchars($e->getMessage()) . "</div>";
        }
    }
    ...
    if (isset($_POST['delete_id'])) {
        ...
        try {
            ...
            $pdo->prepare("DELETE FROM order_detail_management WHERE order_management_id = ?")->execute([$delete_id]);
    ...
    if (isset($_POST['shipping_done'])) {
        try {
            $stmt = $pdo->prepare("UPDATE order_management SET shipping_mail_sent = 1 WHERE order_management_id = ?");
            $stmt->execute([$order_id]);

            $subject = "【RePhone】ご注文発送完了のお知らせ";
            $body = $name . " 様\n\n"
                  . "ご注文ID：" . $order_id . "\n"
                  . "ご注文の商品は本日発送されました。\n\n"
                  . "商品到着まで今しばらくお待ちください。";
            ...
            // alertで表示してページリロード
            echo "<script>alert('発送完了メールを送信しました'); window.location.href='K6-order_detail.php?id={$order_id}';</script>";
            exit;

        } catch (Exception $e) {
            echo "<script>alert('発送メール送信エラー: " . htmlspecialchars($e->getMessage()) . "');</script>";
        }
    }
}
            ...
            <div class="column is-one-third">
                ...
                <?php else: ?>
                    <form method="POST" onsubmit="return confirm('本当にキャンセル解除しますか？');">
                        ...
                        <input type="hidden" name="email" value="<?= htmlspecialchars($order['customer_email']); ?>">
                        <input type="hidden" name="name"  value="<?= htmlspecialchars($order['customer_name']); ?>">
                        <button class="button is-info is-small is-fullwidth" style="margin-top:8px;">キャンセル解除</button>
                    </form>
                <?php endif; ?>

                <!-- キャンセル申請却下ボタン -->
                <?php if (!empty($order['cancel_request_status']) && $order['cancel_request_status'] === '申請中' && empty($order['cancelled_at'])): ?>
                    <form method="POST" onsubmit="return confirm('キャンセル申請を却下しますか？');">
                        <input type="hidden" name="cancel_reject" value="1">
                        <input type="hidden" name="email" value="<?= htmlspecialchars($order['customer_email']); ?>">
                        <input type="hidden" name="name" value="<?= htmlspecialchars($order['customer_name']); ?>">
                        <button class="button is-warning is-small is-fullwidth" style="margin-top:8px;">
                            キャンセル申請を却下する
                        </button>
                    </form>
                <?php endif; ?>
            ...
            <div class="column is-two-thirds">
                <table class="table is-fullwidth is-striped">
                    <tbody>
                        ...
                        <tr>
                            <th>顧客名</th>
                            <td><?= htmlspecialchars($order['customer_name'] ?? '―'); ?> 様</td>
                        </tr>
                        <tr>
                            <th>顧客番号</th>
                            <td><?= htmlspecialchars($order['customer_management_id'] ?? '―'); ?></td>
                        </tr>
                        <tr>
                            <th>電話番号</th>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php if (!empty($order['customer_email'])): ?>
                                    <?php if (!empty($order['shipping_mail_sent'])): ?>
                                        <span class="tag is-success" style="margin-left:10px;">発送完了メール送信済み</span>
                                    <?php else: ?>
                                        <form method="POST" style="display:inline-block; margin-left:10px;">
                                            <input type="hidden" name="shipping_done" value="1">
                                            <input type="hidden" name="email" value="<?= htmlspecialchars($order['customer_email']); ?>">
                                            <input type="hidden" name="name" value="<?= htmlspecialchars($order['customer_name']); ?>">
                                            <button class="button is-success is-small">発送完了メールを送信</button>
                                        </form>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </td>
                        ...
                        <tr>
                            <th>住所</th>
                            <td><?= htmlspecialchars($order['customer_address'] ?? '―'); ?> <?= htmlspecialchars($order['street_address'] ?? ''); ?></td>
                        </tr>
                        <tr>
                            <th>郵便番号</th>
                            <td>
                        ...
                        <tr>
                            <th>配達希望日</th>
                            <td><?= htmlspecialchars($order['delivery_date'] ?: '指定なし'); ?></td>
                        </tr>
                        <tr>
                            <th>配達希望時間</th>
                            <td><?= htmlspecialchars($order['delivery_time'] ?: '指定なし'); ?></td>
                        </tr>
                        <tr>
                            <th>入金状況</th>
                            <td>
                                <span class="tag <?= ($order['payment_confirmation'] === '入金済み') ? 'is-success' : 'is-danger'; ?>">
                                    <?= htmlspecialchars($order['payment_confirmation'] ?? '未入金'); ?>
                                </span>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <?php if (!empty($order['cancel_request_status']) && $order['cancel_request_status'] === '申請中'): ?>
                            <tr><th>キャンセル理由</th><td><?= htmlspecialchars($order['cancel_reason'] ?? '―'); ?></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
